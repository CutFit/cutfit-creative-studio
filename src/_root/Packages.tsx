...
import { packages, packages_adons } from "@/modelDataset";
      ...
      <section className="bg-black text-white py-24 px-6 md:px-16">
        ...
        <motion.div {...fadeUp} className="text-center max-w-3xl mx-auto mb-20">
          <h1 className="text-5xl md:text-6xl font-extrabold mb-6">
             {packages_adons?.title}
          </h1>
          <p className="text-lg text-white/70">
            {packages_adons?.sub_title}
          </p>
        </motion.div>
            ...
            return (
              <motion.div
                ...
                className={`rounded-2xl p-8 border ${
                  pkg.highlight
                    ? "border-[#D7263D] bg-gradient-to-b from-[#14213D]/40 to-[#000] shadow-xl scale-105"
                    : "border-white/10 bg-[#0f0f0f]"
                }`}
              >
                <h3 className="text-2xl font-bold mb-3">{pkg.name}</h3>
                <p className="text-[#D7263D] text-3xl font-extrabold mb-6">
                  {pkg.price}
                </p>
                ...
                <Button
                  className="w-full bg-[#D7263D] text-black hover:bg-[#E5E5E5] py-5 text-base font-semibold"
                  onClick={() => {
                    if (isPro) {
                      navigate("/contact");
                    } else {
        ...
        <motion.div {...fadeUp} className="mt-24 max-w-3xl mx-auto text-center">
          <h2 className="text-4xl font-bold mb-6">{packages_adons?.extra_section_title}</h2>
          {/* <p className="text-white/70 mb-10">
          {packages_adons?.sub_title}
          </p> */}

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-left">
            {packages_adons?.extras.map((item, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
        ...
        <motion.div {...fadeUp} className="mt-32 text-center max-w-2xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button className="bg-[#D7263D] text-black px-6 py-5 text-lg font-semibold hover:bg-[#E5E5E5]"
